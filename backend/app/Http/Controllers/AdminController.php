...
class AdminController extends Controller
{
    ...
    public function statistics(Request $request, AdminSiteStatistics $siteStatistics)
    {
        ...
        return response()->json([
            'data' => $siteStatistics->build($period),
            'meta' => ['period' => $period],
        ]);
    }
    ...
    public function destroyManager(User $manager)
    {
        if ($manager->rule !== 'manager') {
            return response()->json(['message' => 'Удалять можно только менеджеров'], 422);
        }
        ...
        return response()->json(['message' => 'Менеджер успешно удален']);
    }

    public function rollbackAuditLog(AdminAuditLog $auditLog)
    {
        if ($auditLog->reverted_at) {
            return response()->json(['message' => 'Это действие уже было откачено'], 422);
        }
        ...
        if (! $wasRolledBack) {
            return response()->json(['message' => 'Откат не выполнен: целевая запись отсутствует или уже занята'], 422);
        }

        return response()->json(['message' => 'Изменение успешно откачено']);
    }
