...
function ConfirmDeleteModal({ title, name, action, onClose }) {
    return (
        ...
        >
            ...
            >
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                    <a
                        href={`/manager/content/news/${item.id}/preview`}
...
function NewsFormModal({ item, onClose, errors, oldValues }) {
    const isEdit = Boolean(item);
    const action = isEdit ? `/manager/content/news/${item.id}` : "/manager/content/news";
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 px-4 py-6" onClick={onClose}>
            ...
            >
                ...
                <form action={action} method="POST" className="mt-5 space-y-4">
                    ...
                    <button type="submit" className="btn-fill h-[44px] min-w-[210px] bg-white px-6 py-2.5 text-[14px] font-semibold">
                        <span className="relative z-10">{isEdit ? "Сохранить изменения" : "Добавить новость"}</span>
                    </button>
                </form>
            </div>
        </div>
...
function VacancyFormModal({ item, onClose, errors, oldValues }) {
    const isEdit = Boolean(item);
    const action = isEdit ? `/manager/content/vacancies/${item.id}` : "/manager/content/vacancies";
    const defaults = isEdit
        ? item
        : {
              title: oldValues.title || "",
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 px-4 py-6" onClick={onClose}>
            ...
            >
                ...
                <form action={action} method="POST" className="mt-5 space-y-4">
                    ...
                    <button type="submit" className="btn-fill h-[44px] min-w-[210px] bg-white px-6 py-2.5 text-[14px] font-semibold">
                        <span className="relative z-10">{isEdit ? "Сохранить изменения" : "Добавить вакансию"}</span>
                    </button>
                </form>
            </div>
        </div>
...
const ManagerContent = () => {
    const newsPosts = Array.isArray(window.managerNewsPosts) ? window.managerNewsPosts : [];
    const vacancies = Array.isArray(window.managerVacancies) ? window.managerVacancies : [];
    const success = window.managerSuccess || null;
    const errors = window.errors || {};
    const oldValues = window.managerOld || {};
                ...
                <ConfirmDeleteModal
                    ...
                    action={
                        deleteTarget.type === "news"
                            ? `/manager/content/news/${deleteTarget.id}`
                            : `/manager/content/vacancies/${deleteTarget.id}`
                    }
